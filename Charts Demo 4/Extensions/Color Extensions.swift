...
extension Color {
  static func randonColorsN(n: Int) -> [Color] {
...
// Otherwise just return n random colors with some repition (sam eas last func)
extension Color {
  static func randonColorsNDifferent(n: Int) -> [Color] {
    if n > mainColorList.count {
      return Color.randonColorsN(n: n)
    } else {
      var theRandomColors: [Color] = []
      for _ in 0..<n {
        var getNewColor = true
        while (getNewColor) {
          let newColor = Color.random
          if !theRandomColors.contains(newColor) {
            theRandomColors.append(newColor)
            getNewColor.toggle()
          }
        }
      }
